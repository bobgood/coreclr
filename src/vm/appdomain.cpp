...
AppDomain *SystemDomain::GetAppDomainAtId(ADID index)
{
	...
	{
#ifdef _DEBUG
		if (!SystemDomain::IsUnderDomainLock() && !IsGCThread()) { MODE_COOPERATIVE; }
		else { DISABLED(MODE_ANY); }
#endif
		GC_NOTRIGGER;
		SO_TOLERANT;
		NOTHROW;
...
HRESULT SystemDomain::RunDllMain(HINSTANCE hInst, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		NOTHROW;
	if (GetThread() && !lpReserved) { MODE_PREEMPTIVE; }
	else { DISABLED(MODE_PREEMPTIVE); };
...
void SystemDomain::PublishAppDomainAndInformDebugger(AppDomain *pDomain)
{
	...
	{
		if (!g_fEEInit) { THROWS; }
		else { DISABLED(NOTHROW); };
		if (!g_fEEInit) { GC_TRIGGERS; }
		else { DISABLED(GC_NOTRIGGER); };
		MODE_ANY;
	}
	CONTRACTL_END;
...
public:
	...
	LoadDomainAssemblyStress(AppDomain *pThis, AssemblySpec* pSpec, PEAssembly *pFile, FileLoadLevel targetLevel, AssemblyLoadSecurity *pLoadSecurity)
		: pThis(pThis), pSpec(pSpec), pFile(pFile), pLoadSecurity(pLoadSecurity), targetLevel(targetLevel) {
		LIMITED_METHOD_CONTRACT;
	}
...
public:
	...
	LoadDomainModuleStress(AppDomain *pThis, DomainAssembly *pAssembly, PEModule *pFile, FileLoadLevel targetLevel)
		: pThis(pThis), pAssembly(pAssembly), pFile(pFile), targetLevel(targetLevel) {
		LIMITED_METHOD_CONTRACT;
	}
...
HRESULT AppDomain::UnloadById(ADID dwId, BOOL fSync, BOOL fExceptionsPassThrough)
{
	CONTRACTL
	{
		if (fExceptionsPassThrough) { THROWS; }
		else { NOTHROW; }
		MODE_ANY;
		if (GetThread()) { GC_TRIGGERS; }
		else { DISABLED(GC_TRIGGERS); }
		FORBID_FAULT;
	}
	CONTRACTL_END;
